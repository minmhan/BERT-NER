...
flags.DEFINE_string(
    "export_dir", None,
    "The export directory where servable model will be written.")
...
flags.DEFINE_bool("do_serve", False, "Whether to save serving model.")
...
def serving_input_receiver_fn_v2():
    input_ids = tf.placeholder(tf.int64, [None, FLAGS.max_seq_length], name='input_ids')
    mask = tf.placeholder(tf.int64, [None, FLAGS.max_seq_length], name='mask')
    segment_ids = tf.placeholder(tf.int64, [None, FLAGS.max_seq_length], name='segment_ids')
    label_ids = tf.placeholder(tf.int64, [None, FLAGS.max_seq_length], name='label_ids')
...
def main(_):
    ...
    if FLAGS.do_serve:
        logging.info("***** Running export model *****")
        logging.info(FLAGS.export_dir)            
        estimator._export_to_tpu = False
        estimator.export_savedmodel(FLAGS.export_dir, serving_input_receiver_fn_v2)
        logging.info("***** Finished export model*****")
